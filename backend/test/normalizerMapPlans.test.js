...
test('normalizeMapPlans segments agent picks by MAP draft actions when present', () => {
  const teamId = 't1';
      ...
      games: [
        {
          ...
          teams: [
            { id: teamId, name: 'Team A', score: 13, won: true },
            { id: 't2', name: 'Team B', score: 9, won: false },
          ],
        },
        {
          id: 'g2',
          map: { name: 'Bind' },
          teams: [
            { id: teamId, name: 'Team A', score: 9, won: false },
            { id: 't2', name: 'Team B', score: 13, won: true },
          ],
        },
      ],
      draftActions: [
        { type: 'BAN', drafter: { id: teamId }, draftable: { name: 'Haven', type: 'MAP' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Ascent', type: 'MAP' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Jett', type: 'AGENT' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Omen', type: 'AGENT' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Bind', type: 'MAP' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Raze', type: 'AGENT' } },
        { type: 'PICK', drafter: { id: teamId }, draftable: { name: 'Brimstone', type: 'AGENT' } },
      ],
    },
  ...
  const ascent = plans.find(p => p.mapName === 'Ascent');
  assert.ok(ascent);
  assert.equal(ascent.mapPickCount, 1);
  assert.equal(ascent.mapBanCount, 0);
  assert.ok(ascent.commonCompositions);
  assert.equal(ascent.commonCompositions[0].members.join('|'), 'Jett|Omen');

  const bind = plans.find(p => p.mapName === 'Bind');
  assert.ok(bind);
  assert.equal(bind.mapPickCount, 1);
  assert.ok(bind.commonCompositions);
  assert.equal(bind.commonCompositions[0].members.join('|'), 'Brimstone|Raze');

  const haven = plans.find(p => p.mapName === 'Haven');
  assert.ok(haven);
  assert.equal(haven.matchesPlayed, 0);
  assert.equal(haven.winRate, 0);
  assert.equal(haven.mapPickCount, 0);
  assert.equal(haven.mapBanCount, 1);
});
